...
function skeletonFromJsonSchema(schema: Record<string, unknown>): unknown {
  ...
  // A JSON Schema `type` may be an array of alternatives, e.g.
  // `{ type: ["string", "null"] }` - some Zod versions (and non-Zod JSON
  // Schema producers) represent a nullable field this way rather than as
  // `anyOf`. `exampleFromJsonSchema` is a public entry point for schemas
  // generated by whatever tooling the caller uses (see its doc comment),
  // so this shape must be handled here too, not just via the `anyOf`
  // branch above. Normalize to the non-null member so the switch below
  // produces a type-appropriate placeholder instead of always falling
  // through to `null`.
