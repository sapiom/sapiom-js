 ...
 *     "version": 1,
 *     "steps": {
 *       "<stepName>": { "<capability.path>": <response> }
 *     }
 *   }
 *
 * Capability paths are namespace methods (`repositories.list`, `models.coding.run`)
...
/** One step's overrides: capability path → the response returned verbatim. */
...
export function parseStubFile(raw: unknown): StubFile {
  ...
  for (const [stepName, stepStubs] of Object.entries(
    ...
    if (
      ...
      throw invalid(
        `steps.${stepName} must be an object keyed by capability path.`,
      );
    }
