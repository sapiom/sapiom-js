...
describe("CodexAdapter", () => {
  describe("launch/resume", () => {
    ...
    it("translates Studio's generated MCP file into process-local Codex config without exposing headers in argv", async () => {
      const configDir = await mkdtemp(join(tmpdir(), "harness-codex-mcp-"));
      ...
      expect(spec.args).toContain('mcp_servers.sapiom-project.command="npx"');
      expect(spec.args).toContain(
        'mcp_servers.sapiom-project.args=["-y","@sapiom/mcp@latest"]',
      );
      expect(spec.args).toContain(
        'mcp_servers.sapiom-project.env.SAPIOM_ENVIRONMENT="staging"',
      );
      expect(spec.args).toContain(
        'mcp_servers.sapiom-cloud.url="https://api.sapiom.ai/v1/mcp"',
      );
      expect(spec.args).toContain(
        'mcp_servers.sapiom-cloud.env_http_headers={"x-api-key"="SAPIOM_STUDIO_MCP_HEADER_0"}',
      );
      expect(spec.args.join(" ")).not.toContain("secret-value");
      expect(spec.env).toEqual({ SAPIOM_STUDIO_MCP_HEADER_0: "secret-value" });
