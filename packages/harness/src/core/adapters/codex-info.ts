...
const INSTALL_MCP_PROMPT = [
  ...
  "",
  `1. Register Sapiom Project MCP under the client alias \`${PROJECT_MCP_ALIAS}\`. Recent Codex versions`,
  "   support:",
  "",
  `   codex mcp add ${PROJECT_MCP_ALIAS} -- npx -y @sapiom/mcp`,
  "",
  "   Otherwise add it to `~/.codex/config.toml` yourself:",
  "",
  `   [mcp_servers.${PROJECT_MCP_ALIAS}]`,
  '   command = "npx"',
  '   args = ["-y", "@sapiom/mcp"]',
  "",
