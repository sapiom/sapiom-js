...
export interface GenerateClaudeSettingsOptions {
  ...
  /**
   * Native Claude Code notice shown only for a fresh SessionStart event.
   * This is display-only orientation, not model context or a user turn.
   */
 ...
 */
function emitScriptSource(sessionStartSystemMessage?: string): string {
  const displayMessage = sessionStartSystemMessage?.trim() || null;
...
const sessionStartSystemMessage = ${JSON.stringify(displayMessage)};
...
async function main() {
  ...
  // A native SessionStart systemMessage is the only interactive Claude Code
  // channel that can orient the user without manufacturing a model/user turn.
  // Keep it fresh-only: resume/clear/compact already have conversation state.
  if (
    ...
    try {
      require("node:fs").writeFileSync(
        1,
        JSON.stringify({ systemMessage: sessionStartSystemMessage }) + "\\n",
      );
    } catch {
      // Display failure must never prevent the CLI from starting.
    }
  }
}
...
export async function generateClaudeSettings(
  ...
  await fs.writeFile(
    emitScriptPath,
    emitScriptSource(options.sessionStartSystemMessage),
    { mode: 0o755 },
  );
