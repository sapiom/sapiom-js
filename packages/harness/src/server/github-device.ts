 ...
 *
 * The Client ID comes from `SAPIOM_GITHUB_CLIENT_ID` (read when the router is
 * created), with Studio's public OAuth Client ID as the default. When explicitly
 * disabled in tests, the router still mounts and reports `notConfigured`.
 *
 ...
 *  unset — in which case the UI reports that GitHub authorization is
 *  unavailable. */
...
/** Expire the browser's opaque session key after its server-side token is gone. */
function clearSessionKey(res: Response): void {
  res.setHeader(
    "Set-Cookie",
      ...
      try {
        ...
        const rawUri =
          typeof data.verification_uri === "string"
            ...
            : "";
        const safeUri = rawUri.startsWith("https://github.com/")
          ? rawUri
          : GITHUB_DEVICE_URI;
        res.json({
  ...
  router.post("/api/github/device/poll", requireClientId, async (req, res) => {
    ...
    try {
      ...
      if (result.status === "authorized") {
        ...
        const previousSessionKey = readSessionKey(req);
        if (previousSessionKey) tokenStore.delete(previousSessionKey);
      ...
      res
        .status(502)
        .json({ error: `Failed to reach GitHub: ${redactToken(msg)}` });
    }
  });
