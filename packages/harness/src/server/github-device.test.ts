...
describe("mapPollResponse", () => {
  ...
  it("maps error: authorization_pending → pending", () => {
    expect(mapPollResponse({ error: "authorization_pending" })).toEqual({ status: "pending" });
  });
  ...
  it("maps error: expired_token → expired", () => {
    expect(mapPollResponse({ error: "expired_token" })).toEqual({ status: "expired" });
  });

  it("maps error: access_denied → denied", () => {
    expect(mapPollResponse({ error: "access_denied" })).toEqual({ status: "denied" });
  });

  it("maps unknown error → pending (safe default)", () => {
    expect(mapPollResponse({ error: "some_unknown_error" })).toEqual({ status: "pending" });
  });
...
describe("POST /api/github/device/start", () => {
  ...
  it("returns 503 when clientId is null", async () => {
    ...
    const httpRes = await globalThis.fetch(`${server.baseUrl}/api/github/device/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    });
...
describe("POST /api/github/device/poll", () => {
  ...
  it("returns 400 when device_code is missing", async () => {
    const mockFetch = vi.fn();
    server = startServer(mockFetch);
    const res = await globalThis.fetch(`${server.baseUrl}/api/github/device/poll`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
...
describe("GET /api/github/repos", () => {
  ...
  beforeEach(async () => {
    ...
    server = startServer(mockFetch);
    const pollRes = await globalThis.fetch(`${server.baseUrl}/api/github/device/poll`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ device_code: "ghu_abc" }),
    });
...
describe("GET /api/github/status", () => {
  ...
  it("returns { connected: true, login } when session is valid", async () => {
    ...
    const statusRes = await globalThis.fetch(`${server.baseUrl}/api/github/status`, {
...
describe("POST /api/github/disconnect", () => {
  ...
  it("clears the session so subsequent repos requests return 401", async () => {
    ...
    // Disconnect.
    const discRes = await globalThis.fetch(`${server.baseUrl}/api/github/disconnect`, {
      method: "POST",
      headers: { Cookie: cookie },
    });
    ...
    const reposRes = await globalThis.fetch(`${server.baseUrl}/api/github/repos`, {
      headers: { Cookie: cookie },
    });
