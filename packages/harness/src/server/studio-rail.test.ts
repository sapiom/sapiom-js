...
const railFile = (root: string): string =>
  path.join(root, ".sapiom", "studio-rail.json");
...
describe("read / write / remove", () => {
  ...
  it("reads an oversized file as nothing stored rather than loading it", async () => {
    ...
    await fs.writeFile(
      railFile(tmp),
      "x".repeat(MAX_STUDIO_RAIL_BYTES + 1),
      "utf8",
    );
    expect(await readStudioRailFile(tmp)).toBeNull();
  });
...
describe("the routes", () => {
  it("GETs null for a known root with no file, and 400s for an unknown one", async () => {
    ...
    try {
      const ok = await fetch(
        `${url}/api/studio-rail?root=${encodeURIComponent(tmp)}`,
      );
      ...
      const nope = await fetch(
        `${url}/api/studio-rail?root=${encodeURIComponent(os.tmpdir())}`,
      );
      expect(nope.status).toBe(400);
      expect(await fetch(`${url}/api/studio-rail`)).toHaveProperty(
        "status",
        400,
      );
    } finally {
  ...
  it("refuses a relative path and one carrying a `..` segment", async () => {
    ...
    try {
      for (const root of ["relative/path", `${tmp}/../${path.basename(tmp)}`]) {
        const res = await fetch(
          `${url}/api/studio-rail?root=${encodeURIComponent(root)}`,
        );
  ...
  it("serves launch edges without a root, and does not read a file for them", async () => {
    ...
    await fs.writeFile(
      ...
      'await ctx.sapiom.agents.launch({ definition: "queue" });\n',
      "utf8",
    );
    ...
    try {
      ...
      expect(await res.json()).toEqual({
        edges: [{ parent: "gateway", child: "queue" }],
      });
    } finally {
      await close();
    }
...
describe("detectLaunchEdges", () => {
  ...
  it("reads both SDK spellings of a launch call", async () => {
    const workflows = [
      await seed(
        "current",
        'import { agents } from "@sapiom/tools";\nagents.launch({ definition: "child-a" })',
      ),
      await seed(
        "legacy",
        'import { orchestrations } from "@sapiom/tools";\norchestrations.launch({ definition: "child-b" })',
      ),
    ];
  ...
  it("contributes nothing for a project whose directory is gone", async () => {
    expect(
      await detectLaunchEdges([
        { name: "ghost", path: path.join(tmp, "ghost") },
      ]),
    ).toEqual([]);
  });
  ...
  it("reports an edge to a definition no local agent provides — the client filters it", async () => {
    ...
    const workflow = await seed(
      "outreach",
      'import { agents } from "@sapiom/tools";\nagents.launch({ definition: "ghost-agent" })',
    );
    expect(await detectLaunchEdges([workflow])).toEqual([
