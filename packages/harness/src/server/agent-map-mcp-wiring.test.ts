...
it("gives a signed-out local planner its scoped Agent Map tools", async () => {
  ...
  expect(created.session.agentMapIdentity).toEqual(
    expect.objectContaining({ userId: "local:machine-1" }),
  );
  ...
  expect(systemPrompt).toContain("<studio-project-context>");
  expect(systemPrompt).toContain("<studio-project-agent>");
  expect(systemPrompt).toContain(codingPrompt);
  expect(systemPrompt).not.toContain("Do not act as a coding or implementation agent");
  ...
  expect(await fs.readFile(ordinaryLaunch.systemPromptFile!, "utf8")).toContain(
    codingPrompt,
  );
