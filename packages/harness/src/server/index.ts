...
export const startServer = async (
  ...
  const generatedCleanupScheduled = new Set<string>();
  ...
  const buildLaunchOpts: LaunchOptsBuilder = async (
    ...
    // Restored and imported history may be exited without this server having
    // observed its exit. Close that old lifetime before yielding, so metadata
    // broadcasts cannot schedule deletion during this resume's generation.
    // Any removal already started still has to finish before files are written.
    if (sessionManager.get(harnessSessionId)?.status === "exited") {
      generatedCleanupScheduled.add(harnessSessionId);
    }
  ...
  sessionManager.onStatusChange((session) => {
    ...
    if (session.status === "starting" || session.status === "running") {
      generatedCleanupScheduled.delete(session.id);
    }
    if (session.status === "running") {
      ...
      // Metadata broadcasts can repeat status=exited while resume is still
      // building its launch options. Only the first exit broadcast may
      // schedule removal; another would race the freshly generated files
      // after buildLaunchOpts has awaited the original cleanup above.
      if (generatedCleanupScheduled.has(session.id)) return;
      generatedCleanupScheduled.add(session.id);
