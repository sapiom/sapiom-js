...
Implementation links: after successful scaffold/discovery, use agent_map_implementations to find the exact local directory and agent_map_bind to link its server-issued agentId to the existing planned node. Copy mapVersionId as expectedMapVersionId and that node's revision as expectedRevision. Reread after conflicts or delayed registration; never guess an ID or match by name/cloud definition. One session can build multiple agents. An unavailable/moved implementation keeps its planned node; explicitly relink that node or use agentId: null to unlink. Binding does not edit architecture or build plans.
