...
export interface SessionSummary {
  ...
  /** Git branch the session ran on, parsed from the agent's transcript by the
   *  server adapter. Optional: absent on servers that
   *  predate it — row meta degrades to harness + relative time. */
  gitBranch?: string;
  /** Number of user turns in the transcript. Optional for
   *  the same reason as gitBranch; never fabricated client-side. */
  messageCount?: number;
}
...
export type BusMessage =
  ...
  /**
   * A run was started from this session (upstream: ExecutionDetector parses
   * the CLI's "Started execution <id>" line; fires once per distinct
   * (session, executionId)). The SPA starts polling /api/runs/:id/state.
   * Shape mirrors upstream feat/harness-runtime-analytics (bedc3d7) exactly.
   */
  | {
...
// ---------------------------------------------------------------------------
// Runtime analytics — live run render state.
// Copied VERBATIM from upstream feat/harness-runtime-analytics (bedc3d7,
// packages/harness/src/shared/types.ts) so the consumer below lights up
// unchanged when that branch merges. Optional fields are ABSENT on honest
// absence (null cost, still running) — never a fabricated 0.
// ---------------------------------------------------------------------------
...
export interface StepView {
  ...
  error?: string;
  /** Tail-preserving, character-capped executor log text. */
  logSlice?: string;
}
...
export interface HarnessEntry {
   ...
   *  Upstream declares this required; vendored as OPTIONAL because
   *  the harness-launch server (eebb95c) predates the field and omits it —
   *  the composer treats absence as "no image support" so the affordance
   *  self-hides against older servers, never fabricating a capability. */
  imageInput?: boolean;
}
...
/**
 * One user-defined rail group: a named collection of workflow paths,
 * independent of where those workflows live on disk. Purely an organizational
 * projection — deleting a group never touches the workflows themselves.
 */
export interface WorkflowGroupDef {
  /** Stable id (uuid) so renames never orphan memberships. */
  id: string;
  name: string;
  /** Absolute workflow paths filed under this group, in user order. */
  paths: string[];
}
...
export interface HarnessSettings {
  ...
  /**
   * User-defined rail groups. Optional: servers that predate the
   * field strip it from PATCH responses, so the SPA always keeps a
   * localStorage copy as the working fallback and treats this as the
   * durable promotion when the server echoes it back.
   */
  workflowGroups?: WorkflowGroupDef[];
  /**
   * URL template for "Open in editor", with `{path}` standing in
   * for the absolute project path. Absent = the vscode:// default; users on
   * other editors can point it at their own scheme (e.g. cursor://).
   */
