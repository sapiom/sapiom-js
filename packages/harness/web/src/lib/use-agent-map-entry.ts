...
export interface AgentMapEntryState {
  ...
  /** Missing/deleted/foreign projects replace the whole map pane. Every
   * ordinary read failure stays local to the pane. */
...
/**
 * Reads the Agent Map for the selected project from durable state.
 *
 * Opening a project starts nothing (SAP-3143): the map is a read-only view of
 * what the store holds, and the project's sessions are ordinary sessions the
 * user starts explicitly. A late response from a project the user already left
 * is ignored.
 */
export function useAgentMapEntry({
  ...
  const loadWorkspace = useCallback((target: StudioProjectId): void => {
    ...
    setState((current) => ({
      ...(current.projectId === target
        ? current
        : { projectId: target, unavailable: null }),
      projectId: target,
  ...
  useEffect(() => {
    ...
    // The ref survives React StrictMode's setup/cleanup probe, preventing
    // duplicate entry telemetry for one visible visit.
  ...
  // The map is the only pane this hook owns now; "retry all" is kept as the
  // verb the whole-workspace error state calls.
  const retryAll = retryWorkspace;
