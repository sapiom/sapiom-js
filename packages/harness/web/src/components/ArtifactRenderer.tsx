...
export function artifactTextPreview(text: string): ArtifactTextPreview {
  ...
  const truncated =
    lines.length > TEXT_PREVIEW_LINES || text.length > bounded.length;
  return {
    kind: looksLikeHtml(text) ? "html" : "text",
    truncated,
    preview: truncated ? `${bounded.trimEnd()}\n…` : text,
    omittedCharacters: truncated
      ? Math.max(0, text.length - bounded.length)
      : 0,
  };
}
...
function MediaValue({ url }: { url: URL }): JSX.Element {
  ...
  const image = ["png", "jpg", "jpeg", "gif", "webp", "avif", "svg"].includes(
    ext,
  );
...
function isCompositeValue(
  value: unknown,
): value is Record<string, unknown> | unknown[] {
  return value !== null && typeof value === "object";
}

function StructuredValue({ value }: { value: unknown }): JSX.Element {
  if (typeof value === "string") {
    const url = safeUrl(value);
    return url ? (
      <MediaValue key={url.href} url={url} />
    ) : (
  ...
  if (Array.isArray(value)) {
    if (value.length === 0) return <code className="artifact-scalar">[]</code>;
        ...
        {rest.length > 0 && (
          <details className="artifact-collection-more">
            ...
            {rest.map((item, index) => (
              ...
              >
                <span className="artifact-list-index">
                  {index + COLLECTION_PREVIEW + 1}
                </span>
                <StructuredValue value={item} />
              </div>
            ))}
  ...
  if (typeof value === "object") {
    ...
    if (entries.length === 0)
      return <code className="artifact-scalar">{"{}"}</code>;
    ...
    const rows = (items: Array<[string, unknown]>): JSX.Element[] =>
      items.map(([key, child]) => (
        <div
          className="artifact-field"
          data-composite={isCompositeValue(child) || undefined}
          key={key}
        >
          <div className="artifact-field-label">
            {key.replaceAll(/[_-]+/g, " ")}
          </div>
          <div className="artifact-field-value">
...
export function ArtifactRenderer({
  ...
  defaultOpen?: boolean;
  onViewed?: () => void;
  /** Identity of the rendered artifact for once-per-artifact view reporting. */
  viewEventKey?: string;
}): JSX.Element {
  ...
  const bodyId = useId();
  const renderedTabId = useId();
  const rawTabId = useId();
  const renderedTab = useRef<HTMLButtonElement | null>(null);
  const rawTab = useRef<HTMLButtonElement | null>(null);
  const raw = formatPayload(value);
  useEffect(() => {
    if (!onViewed || reportedFor.current === viewEventKey) return;
    reportedFor.current = viewEventKey;
    onViewed();
  }, [onViewed, viewEventKey]);
  useEffect(
    () => () => {
  ...
  const selectMode = (next: "rendered" | "raw"): void => {
    setMode(next);
    setOpen(true);
  };
  const onModeKeyDown = (
    ...
    let next: "rendered" | "raw" | null = null;
    if (event.key === "Home") next = "rendered";
    if (event.key === "End") next = "raw";
    if (
      ["ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown"].includes(event.key)
    ) {
      next = mode === "rendered" ? "raw" : "rendered";
    }
    if (next === null) return;
    event.preventDefault();
    selectMode(next);
    (next === "rendered" ? renderedTab : rawTab).current?.focus();
  };
  return (
    <section
    ...
    >
      ...
      <div
        ...
        id={bodyId}
        role="tabpanel"
        aria-labelledby={mode === "rendered" ? renderedTabId : rawTabId}
        hidden={!open}
      >
        {mode === "rendered" ? (
          <StructuredValue value={value} />
        ) : (
          <TextValue text={raw} forceSource />
        )}
      </div>
    </section>
  );
