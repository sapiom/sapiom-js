...
test.describe("first run", () => {
  ...
  test("greets, says what Studio is, and offers the two ways in", async ({
    page,
  }) => {
    ...
    await expect(panel).toContainText(
      "Local Run replaces Sapiom capability calls with stubs",
    );
    await expect(panel).toContainText("your code still runs on this machine");
    ...
    await expect(folder).toContainText(
      "Agents in the folder appear in the rail",
    );
    ...
    await page.screenshot({
      path: "web/e2e/screenshots/welcome-panel.png",
      fullPage: true,
    });
  });
  ...
  test("documentation is a way out to the docs, never a dismiss", async ({
    page,
  }) => {
...
test.describe("returning user", () => {
  test("the lived-in fixtures render straight into the boot session, no panel", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.locator(".rail-workflows")).toBeVisible();
  ...
  test("Overview drops the first-run greeting but keeps the same anatomy", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.locator(".rail-workflows")).toBeVisible();
  ...
  test("carries no screenshot of itself, and never puts content out of reach", async ({
    page,
  }) => {
    ...
    for (const height of [1000, 640, 460]) {
      ...
      expect(
        card!.height,
        `card must fit the scrim at ${height}px`,
      ).toBeLessThanOrEqual(scrim!.height);
      ...
      await expect(
        page.getByTestId("welcome-browse-templates"),
      ).toBeInViewport();
      ...
      await expect(
        lastRow,
        `last row must be reachable at ${height}px`,
      ).toBeInViewport();
    }

    await page.setViewportSize({ width: 1280, height: 720 });
    await page.screenshot({
      path: "web/e2e/screenshots/welcome-overview.png",
      fullPage: true,
    });
  });

  test("lists workspaces from session history, not just launch dirs", async ({
    page,
  }) => {
  ...
  test("rows carry their agent count, and claim nothing for a bare folder", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.locator(".rail-workflows")).toBeVisible();
    ...
    await expect(page.getByTestId("welcome-recent-acme-app")).toContainText(
      "1 agent",
    );
    // scratch is bare — a first-class workspace with nothing in it yet, so no
    // count is claimed for it.
    await expect(page.getByTestId("welcome-recent-scratch")).not.toContainText(
      "agent",
    );
  ...
  test("clicking a recent workspace opens a session in it", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.locator(".rail-workflows")).toBeVisible();
  ...
  test("rows disable while one is opening, so a second click cannot double-fire", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(page.locator(".rail-workflows")).toBeVisible();
...
test.describe("templates browser, reached from Overview", () => {
  test("shows the live catalog with facets, not a pinned pair", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId("template-card-cold-outreach-engine"),
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId("templates-category-revenue-marketing"),
    ).toContainText("Revenue and marketing");
    await expect(
      page.getByTestId("templates-category-data-knowledge"),
    ).toBeVisible();
