 ...
 *  8. The options menu holds Group by and Sort by only; Past sessions opens
 *     from the history glyph (§4.7, Q9).
 */
...
test.describe("the two verbs", () => {
  test("New project (web): the folder step, then the screen scoped to the folder, and no session", async ({
    ...
    expect(before.createSessionCalls).toEqual([]);
    expect(before.createOrder).toEqual([]);
