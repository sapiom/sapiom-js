 ...
 * ```typescript
 * if (isMCPPaymentError(error)) {
 *   const payment = extractPaymentFromMCPError(error);
 *   console.log(`Payment required: ${payment.accepts[0].amount} ${payment.accepts[0].unit}`);
 * }
 * ```
 */
export function extractPaymentFromMCPError(error: any): X402PaymentResponse {
 ...
 *
 * Follows the same pattern as HTTP adapters (PaymentHandler.ts:150).
 *
 ...
 * @internal
 */
export function getPaymentAuthFromTransaction(transaction: any): string {
