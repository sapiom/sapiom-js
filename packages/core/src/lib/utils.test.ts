...
describe('Header Utilities', () => {
  describe('getHeader', () => {
    ...
    it('should find X-Sapiom-Transaction-Id with any case', () => {
      expect(getHeader({ 'X-Sapiom-Transaction-Id': 'tx_123' }, 'x-sapiom-transaction-id')).toBe('tx_123');
      expect(getHeader({ 'x-sapiom-transaction-id': 'tx_456' }, 'X-Sapiom-Transaction-Id')).toBe('tx_456');
    });
  });
  ...
  describe('setHeader', () => {
    ...
    it('should remove all case variants when setting same header', () => {
      ...
      const customKeys = Object.keys(result).filter((k) => k.toLowerCase() === 'x-custom');
