...
import { TransactionAuthorizer, TransactionDeniedError, TransactionTimeoutError } from './TransactionAuthorizer';
...
describe('TransactionAuthorizer', () => {
  ...
  describe('createAndAuthorize', () => {
    ...
    it('waits for authorization and calls success callback', async () => {
      ...
      expect(onAuthorizationPending).toHaveBeenCalledWith('tx-123', 'test-resource');
      expect(onAuthorizationSuccess).toHaveBeenCalledWith('tx-123', 'test-resource');
      expect(tx.status).toBe('authorized');
    });
    ...
    it('throws TransactionDeniedError when transaction is denied', async () => {
      ...
      expect(onAuthorizationDenied).toHaveBeenCalledWith('tx-123', 'test-resource', expect.any(String));
    });
  ...
  describe('waitForExisting', () => {
    ...
    it('throws when existing transaction is denied', async () => {
      ...
      await expect(authorizer.waitForExisting('tx-existing')).rejects.toThrow(TransactionDeniedError);
    });
    ...
    it('throws TransactionTimeoutError on timeout', async () => {
      ...
      await expect(authorizerFastTimeout.waitForExisting('tx-timeout')).rejects.toThrow(
