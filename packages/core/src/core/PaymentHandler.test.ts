...
import { PaymentTransactionResponse, TransactionResponse, TransactionStatus } from '../types/transaction';
...
describe('PaymentHandler', () => {
  ...
  describe('handlePaymentError', () => {
    ...
    it('should return null if already retried', async () => {
      ...
      const result = await handler.handlePaymentError(error, request, mockRequestExecutor);
    ...
    it('should create transaction and retry with authorized payment', async () => {
      ...
      const result = await handler.handlePaymentError(error, error.request!, mockRequestExecutor);
    ...
    it('should wait for pending transaction authorization', async () => {
      ...
      expect(config.onPaymentRequired).toHaveBeenCalledWith('tx_pending', pendingTransaction.payment);
    ...
    it('should handle authorization timeout', async () => {
      ...
      const result = await quickHandler.handlePaymentError(error, error.request!, mockRequestExecutor);
    ...
    it('should throw if authorized transaction missing authorizationPayload (not swallow)', async () => {
      ...
      await expect(handler.handlePaymentError(error, error.request!, mockRequestExecutor)).rejects.toThrow(
      ...
      expect(config.onPaymentFailed).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining('missing payment authorization payload'),
        }),
      );
    });
    ...
    it('should extract service name from resource URL', async () => {
      ...
      await handler.handlePaymentError(error, error.request!, mockRequestExecutor);
    ...
    it('should reauthorize with payment if existing transaction was authorized without payment', async () => {
      ...
      mockTransactionAPI.get.mockResolvedValue(existingTransaction);
      mockTransactionAPI.reauthorizeWithPayment.mockResolvedValue(reauthorizedTransaction);
      mockRequestExecutor.mockResolvedValue({
        status: 200,
        statusText: 'OK',
        headers: {},
    ...
    it('should handle staggered concurrent requests with reference counting', async () => {
      ...
      // Start first request
      const promise1 = handler.handlePaymentError(error, error.request!, mockRequestExecutor);

      // Start second request after small delay (staggered)
      await new Promise((resolve) => setTimeout(resolve, 10));
      const promise2 = handler.handlePaymentError(error, error.request!, mockRequestExecutor);

      // Start third request after another delay
      await new Promise((resolve) => setTimeout(resolve, 10));
      const promise3 = handler.handlePaymentError(error, error.request!, mockRequestExecutor);
