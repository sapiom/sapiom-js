import { HttpError, HttpRequest, HttpResponse, SapiomTransactionMetadata } from '../http/types';
import {
  extractPaymentData,
...
import { PaymentTransactionResponse, TransactionStatus } from '../types/transaction';
...
export interface PaymentHandlerConfig {
  ...
  onPaymentRequired?: (transactionId: string, payment: PaymentTransactionResponse) => void;
  ...
  async handlePaymentError(
    ...
    try {
      ...
      if (existingTransactionId) {
        transaction = await this.config.sapiomClient.transactions.get(existingTransactionId);

        // If transaction exists but doesn't require payment, reauthorize with payment
        if (!transaction.requiresPayment && transaction.status === TransactionStatus.AUTHORIZED) {
          transaction = await this.config.sapiomClient.transactions.reauthorizeWithPayment(
            existingTransactionId,
            paymentData,
          );
        }
      } else {
        const service = userMetadata?.serviceName || this.extractServiceName(resource);
      ...
      if (transaction.status === TransactionStatus.DENIED || transaction.status === TransactionStatus.CANCELLED) {
        this.config.onPaymentFailed?.(new Error(`Transaction ${transaction.status}: ${transaction.id}`));
      ...
      if (transaction.status !== TransactionStatus.AUTHORIZED) {
        ...
        if (authResult.status !== 'authorized') {
          this.config.onPaymentFailed?.(new Error(`Payment ${authResult.status}: ${transaction.id}`));
      ...
      if (!authorizationPayload) {
        throw new Error(`Transaction ${transaction.id} is authorized but missing payment authorization payload`);
      }
      ...
      const paymentHeaderValue =
        typeof authorizationPayload === 'string'
          ...
          : Buffer.from(JSON.stringify(authorizationPayload)).toString('base64'); // Encode object

      // 9. Retry original request with X-PAYMENT header (case-insensitive safe)
      const retryRequest: HttpRequest = {
        ...originalRequest,
        headers: setHeader(originalRequest.headers, 'X-PAYMENT', paymentHeaderValue),
        metadata: {
