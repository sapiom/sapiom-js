...
describe("runSignal", () => {
  ...
  it("keeps a present-but-empty message under --json", async () => {
    jest.mocked(signal).mockResolvedValue({ matched: 1, message: "" });
    setJsonMode(true);
    const out = captureStdout();
    ...
    // Present is present: `--json` mirrors the SDK result rather than second-
    // guessing which values are worth reporting.
    expect(payload).toEqual({ ok: true, matched: 1, message: "" });
    out.restore();
  });
