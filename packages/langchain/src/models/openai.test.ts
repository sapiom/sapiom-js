 ...
 */
function mockGenerate(content: string, usage?: { input_tokens: number; output_tokens: number; total_tokens: number }) {
...
describe('SapiomChatOpenAI', () => {
    ...
    mockClient = {
      transactions: {
        create: jest.fn().mockResolvedValue({
          ...
          actionName: 'generate',
          resourceName: 'gpt-4',
          costs: [{ id: 'cost-estimate-123', fiatAmount: '0.030', isEstimate: true }],
        }),
  ...
  it('creates SapiomChatOpenAI with Sapiom tracking', () => {
    const model = new SapiomChatOpenAI({ model: 'gpt-4', openAIApiKey: 'test-key' }, { sapiomClient: mockClient });
  ...
  it('works as drop-in replacement for ChatOpenAI', () => {
    ...
    expect((model as any).model || (model as any).modelName).toBe('gpt-4-turbo');
  ...
  it('tracks invoke calls', async () => {
    ...
    expect(createCall.requestFacts.request.estimatedInputTokens).toBeGreaterThan(0);

    // Verify response facts sent
    expect(mockClient.transactions.addFacts).toHaveBeenCalled();
    const addFactsCalls = (mockClient.transactions.addFacts as jest.Mock).mock.calls;
  ...
  it('auto-generates trace ID when not provided', async () => {
    ...
    mockGenerate('Hi', { input_tokens: 10, output_tokens: 5, total_tokens: 15 });
  ...
  it('supports per-invoke trace override', async () => {
    ...
    expect((mockClient.transactions.create as jest.Mock).mock.calls[0][0].traceExternalId).toBe('default-trace');
    ...
    expect((mockClient.transactions.create as jest.Mock).mock.calls[1][0].traceExternalId).toBe('override-trace');
    ...
    expect((mockClient.transactions.create as jest.Mock).mock.calls[2][0].traceExternalId).toBe('default-trace');
  ...
  it('estimates tokens and includes in request facts', async () => {
    ...
    expect(createCall.requestFacts.request.tokenEstimationMethod).toBe('tiktoken');
  });
  ...
  describe('Trace Integration', () => {
    it('reuses auto-generated trace across multiple invokes', async () => {
      ...
      // First invoke
      await model.invoke('First');
      expect((mockClient.transactions.create as jest.Mock).mock.calls[0][0].traceExternalId).toBe(autoGenTraceId);

      // Second invoke - should use same auto-generated trace
      await model.invoke('Second');
      expect((mockClient.transactions.create as jest.Mock).mock.calls[1][0].traceExternalId).toBe(autoGenTraceId);
  ...
  describe('Facts Tracking', () => {
    it('submits request facts with token estimate', async () => {
      const model = new SapiomChatOpenAI({ model: 'gpt-4' }, { sapiomClient: mockClient });

      mockGenerate('Response', { input_tokens: 100, output_tokens: 50, total_tokens: 150 });
    ...
    it('includes call site in request facts', async () => {
      const model = new SapiomChatOpenAI({ model: 'gpt-4o' }, { sapiomClient: mockClient });

      mockGenerate('Response', { input_tokens: 1000, output_tokens: 500, total_tokens: 1500 });
    ...
    it('does not fail invoke if response facts submission fails', async () => {
      ...
      (mockClient.transactions.addFacts as jest.Mock).mockRejectedValueOnce(new Error('Network error'));

      mockGenerate('Response', { input_tokens: 10, output_tokens: 5, total_tokens: 15 });
    ...
    it('submits aggregate response facts for batch generate', async () => {
      ...
      const addFactsCall = (mockClient.transactions.addFacts as jest.Mock).mock.calls[0][1];
  ...
  describe('wrapChatOpenAI', () => {
    it('wraps existing ChatOpenAI instance with Sapiom tracking', () => {
      ...
      const wrappedModel = wrapChatOpenAI(originalModel, { sapiomClient: mockClient });
    ...
    it('wraps model with all LangChain methods available', () => {
      const originalModel = new ChatOpenAI({ model: 'gpt-4', openAIApiKey: 'test-key' });
      const wrappedModel = wrapChatOpenAI(originalModel, { sapiomClient: mockClient });
      ...
      expect(typeof (wrappedModel as any).withStructuredOutput).toBe('function');
    });
  });
});
