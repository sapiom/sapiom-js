...
function mockGenerate(content: string, usage?: { input_tokens: number; output_tokens: number; total_tokens: number }) {
  return jest.spyOn(ChatAnthropicMessages.prototype, 'generate').mockResolvedValue({
    generations: [
      [
        {
          message: {
...
describe('SapiomChatAnthropic', () => {
  ...
  it('tracks invoke calls', async () => {
    ...
    mockGenerate('Hello!', { input_tokens: 10, output_tokens: 5, total_tokens: 15 });
    ...
    expect(createCall.requestFacts.request.modelClass).toBe('SapiomChatAnthropic');
    expect(createCall.requestFacts.request.modelId).toBe('claude-3-5-sonnet-20241022');
  ...
  describe('Double Authorization Prevention', () => {
    ...
    it('authorizes when generate is called directly', async () => {
      ...
      await model.generate([[{ role: 'user', content: 'Direct generate call' }] as any]);
  ...
  describe('wrapChatAnthropic', () => {
    it('wraps existing ChatAnthropic instance with Sapiom tracking', () => {
      ...
      const wrappedModel = wrapChatAnthropic(originalModel, { sapiomClient: mockClient });
