...
- 5808e8f: `contentGeneration`: an async image or video job that terminally FAILS is now distinguishable from one that is merely slow (SAP-3097).
...
- cfa6598: Two `run_local` stub fixes, both needed for a template that reads structured output to trace its graph locally.
