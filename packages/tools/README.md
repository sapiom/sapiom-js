...
| `llm`               | Routed LLM calls: one-shot `run`, deferred `submit` / sessions, and `decide` (probabilities over a fixed answer set) | [src/llm](./src/llm/index.ts) · [decide](./src/llm/decide.ts) |
...
Ask every independent question over the same `state` in one call; they are evaluated in parallel. Keep arithmetic and date math in code — `decide` judges, it does not compute.
