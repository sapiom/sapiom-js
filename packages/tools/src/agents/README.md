...
- **`wait` rides out rate limits and blips; it never fails a run on one.** The agents routes are rate-limited per client IP, and every sandbox shares one egress IP, so a fleet of parents polling their children can be told `429` in unison. A transient fault on a status read — `429`/`408`, any `5xx`, or a transport error such as `fetch failed` / `ECONNRESET` — does not end a `wait` (or a `run`). The loop backs off instead: 2 s, then 4 s, 8 s, … capped at 30 s, with up to 20% jitter so the fleet spreads out, and a `Retry-After` header from the platform replaces the computed delay when one is sent. A successful read resets the schedule to `pollMs`. It gives up with `"unknown"` (carrying the last fault as its `error`) only after **12 consecutive** faults — about four and a half minutes on the default schedule — and no back-off ever outlasts `timeoutMs`: a `"timed_out"` result reached mid-storm carries the interrupting fault in `error.details`. A permanent fault (`404` — the run is gone; `401`/`403`; a malformed body) resolves `"unknown"` at once. Tune it with `wait({ retry: { initialBackoffMs, maxBackoffMs, maxConsecutiveFaults } })` (`AgentWaitRetryOptions`). `status()` retries the same way but only three times, then throws.

  The polling is what gets rate-limited, so from inside a step prefer **`launch` + `pauseUntilSignal`** for a long child: the step suspends and resumes on the child's result signal, making **no poll calls at all** — it can't be throttled, and holds no sandbox while it waits. `wait`/`run` are for standalone, inline use.
