 ...
 * fields. Coding servers currently emit `served_class`/`lane`/`cost_usd` as
 * null (they cannot observe class/lane yet; cost_usd is deprecated) and older
 * servers omit them entirely — both must map to null (unknown), never a
 * fabricated value.
 */
...
describe("coding.run — terminal result mapping", () => {
  it("maps null disclosure fields (current coding servers) to null", async () => {
    const sapiom = createClient({
      apiKey: "k",
      fetch: fakeCodingFetch({ ...BASE_WIRE, served_class: null, lane: null, cost_usd: null }),
    });
