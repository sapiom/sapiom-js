/**
 * Sapiom Node HTTP Demo
 *
 * Demonstrates @sapiom/node-http integration with a real-world workflow:
...
async function main() {
  ...
  log("  Sapiom SDK Demo: @sapiom/node-http", "bright");
  log("═".repeat(60), "bright");
  ...
  try {
    ...
    const response = await client.request<{
      ...
      method: "POST",
      url: `${baseUrl}/api/campaigns/analytics`,
      headers: { "Content-Type": "application/json" },
      body: { campaignId },
    });
  ...
  logInfo("• Uses native Node.js http/https modules");
