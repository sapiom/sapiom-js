...
async function main() {
  ...
  // 1) RECALL — pull prior conventions for this scope (x402-paid)
  ...
  // 5) FORGET, then FORGET AGAIN — second call should 404 (hard delete is not idempotent)
  await memory.forget(appended.id);
  console.log(`forget  -> deleted ${appended.id}`);
  try {
    await memory.forget(appended.id);
    console.log("forget2 -> UNEXPECTED: second forget succeeded (should 404)");
  } catch (err) {
    console.log(`forget2 -> 404 as expected: ${(err as Error).message}`);
  }
}
