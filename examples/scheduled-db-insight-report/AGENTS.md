...
- **A model's free text never becomes a query parameter unchecked.** `detectAnomalies` returns an `Anomaly` with a `table` field the model chose; `readAnomaly` only trusts it when it names a label the snapshot's OWN metrics actually returned (see `knownLabels`), and otherwise reads it as `null` — a value the schema lets the model pick itself, and one `followUp` already skips the query on. `followUp` then reads a trusted table with a parameterized query (`sql.unsafe(FOLLOW_UP_SQL, [table])`) — never string-interpolated — against `pg_stat_user_tables`, a system catalog, so it's safe even if the trust check somehow let something odd through.
- **An anomaly this template can't read is a failed step.** `detectAnomalies` uses `output` + `structuredOf`, and `readAnomaly` throws rather than substituting anything. `deterministicAnomaly` is still there, but ONLY for a snapshot with no metrics to send a model at all — presenting its computed top-value as the analyst model's finding is the unmarked substitution this replaced (SAP-2892).
