...
- **Config is not a secret.** The recipient is ordinary run input (`deliverTo`, declared as a `settings[]` entry in `template.json`), not a vault key. With none set, `summary` returns the recap and says nothing was emailed.
...
- **run_local** — runs your **real** step code against **stub capabilities**, so `models.run` / `database` / `email` return built-in defaults; the trace creates no Sapiom capability spend. Pass `dryRun: true` so `upsert` skips the (stubbed) DB and `summary` skips the (stubbed) send and returns the recap. Returns a per-step trace.
