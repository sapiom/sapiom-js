// =============================================================================
// Newsletter issue dashboard — the page this template ships beside its agent.
//
// One dependency-free Node server (`node server.mjs`, no build step) that serves
// `index.html` and one JSON route, `/api/issue`, carrying the latest completed
// run's terminal output: the issue the agent wrote (subject, markdown body,
// header image), the sources it kept after dedupe, the self-edit grade, and
// the delivery facts. The page renders that and nothing else — every number on
// screen is a number the agent produced.
//
...
//             is served instead: the verbatim output of a real run of this
//             template with no subscribers set, so the page is never empty and
//             never invented.
//
// The response always says which (`source.kind`) — the page prints it, so a
// reader can tell a live issue from the captured one at a glance.
// =============================================================================
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/issue") {
      const body = JSON.stringify(await issue());
      res.writeHead(200, {
...
server.listen(PORT, () => {
  ...
  console.log(
    `newsletter issue dashboard on http://localhost:${PORT} (${mode})`,
  );
});
