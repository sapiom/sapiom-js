# Newsletter issue dashboard

The page this template ships beside its agent: the latest issue as the agent
wrote it (header image, title, sections, sources), the sources it kept after
dedupe, the self-edit grade, and the delivery facts — every number on it
produced by a run of the agent.
...
page (`index.html`) and one JSON route (`/api/issue`).
...
| Either missing, or the live read fails              | **Captured** — `sample-report.json`, the verbatim output of a real run of this template with no subscribers set. Never empty, never invented.                 |
...
after a visual change: `pnpm examples:app:screenshot newsletter-autopilot`.
