 ...
 * Sapiom Axios Demo
 *
 * Demonstrates @sapiom/axios integration with a real-world workflow:
...
async function main() {
  ...
  log("  Sapiom SDK Demo: @sapiom/axios", "bright");
  log("═".repeat(60), "bright");
  ...
  try {
    ...
    const response = await client.get("/api/crm/customers", {
      params: { limit: 3, segment: "enterprise" },
    });
  ...
  for (const customer of customers) {
    try {
      ...
      const response = await client.post("/api/sms", {
      ...
      smsCount++;
      logSuccess(`SMS sent - ID: ${response.data.messageId}`);
      logInfo(`Status: ${response.data.status}, Price: $${response.data.price}`);
    } catch (error: any) {
  ...
  try {
    ...
    const response = await client.post("/api/campaigns/analytics", {
      campaignId,
    });

    logSuccess("Analytics retrieved successfully");
    logInfo(`Campaign: ${response.data.name}`);
    logInfo(`Status: ${response.data.status}`);
    if (response.data.metrics) {
      logInfo(`Messages sent: ${response.data.metrics.sent}`);
      logInfo(`Delivered: ${response.data.metrics.delivered}`);
      logInfo(`Open rate: ${response.data.metrics.openRate}%`);
    }
  } catch (error: any) {
  ...
  logInfo("• No changes needed to your existing Axios code");
