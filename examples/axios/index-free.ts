/**
 * Sapiom Axios Demo - Free Endpoints Only
 *
 * Demonstrates @sapiom/axios with free endpoints that only require authorization:
...
async function main() {
  ...
  log("  Sapiom SDK Demo: @sapiom/axios (Free Endpoints)", "bright");
  log("═".repeat(60), "bright");
  ...
  try {
    ...
    const response = await client.get("/api/public/time");
  ...
  try {
    ...
    const response = await client.get("/api/public/status");
