...
- **run_local** — runs your **real** step code against **stub capabilities**, so `models.run` / `database` / `email` return built-in defaults; the trace creates no Sapiom capability spend. Pass `dryRun: true` so `dedupe` skips the (stubbed) DB and `digest` skips the (stubbed) send and returns the preview. Returns a per-step trace.
