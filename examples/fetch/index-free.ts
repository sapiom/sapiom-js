/**
 * Sapiom Fetch Demo - Free Endpoints Only
 *
 * Demonstrates @sapiom/fetch with free endpoints that only require authorization:
...
async function main() {
  ...
  log("  Sapiom SDK Demo: @sapiom/fetch (Free Endpoints)", "bright");
  log("═".repeat(60), "bright");
  ...
  try {
    ...
    const response = await safeFetch(`${baseUrl}/api/public/time`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    const data = await response.json();

    logSuccess(`Server time: ${data.time}`);
    logInfo(`Timezone: ${data.timezone}`);
  } catch (error: any) {
  ...
  try {
    ...
    const response = await safeFetch(`${baseUrl}/api/public/status`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    const data = await response.json();
