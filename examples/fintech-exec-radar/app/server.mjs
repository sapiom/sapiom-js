// =============================================================================
// Opportunity radar dashboard: the page this template ships beside its agent.
//
...
// radar run's terminal output: the sourced items, each company's coverage, the
// run-health counters, and the degraded steps it recorded. The page renders
// that and nothing else: every headline and every count on screen is one the
// agent returned.
//
// Two data sources, chosen by environment:
//
//   live      SAPIOM_API_KEY + SAPIOM_DEFINITION_ID are set. The latest completed
//             radar run of that deployed agent is read over the Sapiom API and
//             cached for a short while. The publish step that puts this
//             dashboard on an App Link is expected to inject both
//             (SAPIOM_API_URL optional, defaults to production).
//   captured  Either is missing, or the live read fails. `sample-run.json` is
//             served instead: the terminal output of a real run of this
//             template on its built-in watchlist, so the page is never empty
//             and never invented.
//
...
/**
 * The agent's research children run under the same definition as the radar
 * run that launched them, and finish before it, so the newest completed run is
...
function isRadarRun(run) {
  return (
    run != null &&
    typeof run === "object" &&
    run.parentExecutionId == null &&
    run.output != null &&
    typeof run.output.digest === "string"
  );
}

/**
 * Latest completed radar run of the bound agent, as `{ source, output }`.
...
async function readLive() {
  ...
  const rows = await api(`/v1/workflows/executions?${query}`);
  if (!Array.isArray(rows) || rows.length === 0) {
    throw new Error("no completed run yet");
  }
  const candidates = rows
    .filter((row) => !CHILD_KEY.test(String(row?.idempotencyKey ?? "")))
    .slice(0, MAX_DETAIL_READS);
  let run = null;
  for (const row of candidates) {
    const detail = await api(
      `/v1/workflows/executions/${encodeURIComponent(row.id)}`,
    );
    if (isRadarRun(detail)) {
      run = detail;
      break;
    }
  }
...
server.listen(PORT, () => {
  ...
  console.log(
    `opportunity radar dashboard on http://localhost:${PORT} (${mode})`,
  );
});
