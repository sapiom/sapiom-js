...
type WorkflowInputContractResponse =
  | { status: "available"; jsonSchema: Record<string, unknown>; example: unknown }
  | { status: "none"; jsonSchema: null; example: Record<string, never> }
  | { status: "unavailable"; jsonSchema: null; example: Record<string, never>; reason: string };
...
- Debug: `attempt selection → extractStepContext → bounded prompt → injectInput`.

## Test plan

- Contract route distinguishes available/none/unavailable and never reads an unregistered path.
- Form covers examples/defaults/skeletons, nested/scalar arrays, stale saved input, JSON mode, validation, and exact launch payloads.
...
2. A dedicated contract endpoint avoids coupling Run to iframe-posted graph state and works before the Canvas document mounts.
