...
| Removal tombstones | `ui-prefs.closedProjects` | A project the user REMOVED from the rail comes straight back on the next launch, wearing the cwd of a session that already ended. Two halves (`recentDirs` server-side, the tombstone client-side), two places, two lifetimes | **Open** — leftover F5 in `plans/studio-rail-rebuild/SESSION-HANDOFF-2026-08-28.md` §6, called out there as the highest-value leftover. Needs a server endpoint, not a settings field: it is per-project |
